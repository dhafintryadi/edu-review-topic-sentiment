"""
src/run_phase3c1.py
Phase 3C-1: Focal Loss Experiment
Trains IndoBERTweet with FocalLoss (gamma=2.0) on the same cleaned dataset,
with the same architecture and seed as the Phase 3A baseline.
Variable changed: CrossEntropyLoss → FocalLoss(gamma=2.0)
"""
...
LOG_FILE = os.path.join(LOGS, "phase3c1.log")
...
EXPERIMENT_METADATA = {
    ...
    "layer_freezing": "embeddings + encoder.layer.0-7",
}


def main():
    logger.info("=" * 60)
    logger.info("PHASE 3C-1 - FOCAL LOSS EXPERIMENT")
    logger.info(f"Timestamp: {datetime.datetime.now().isoformat()}")
    logger.info(f"Variable: CrossEntropyLoss → FocalLoss(gamma={EXPERIMENT_METADATA['focal_gamma']})")
    logger.info("=" * 60)
    ...
    history_df.to_csv(os.path.join(RESULTS, "focal_training_history.csv"), index=False)
    ...
    if os.path.exists(BASELINE_METRICS_PATH):
        ...
        focal_metrics["delta_macro_f1"] = focal_metrics["test_macro_f1"] - baseline.get("test_f1_macro", 0)
        focal_metrics["delta_accuracy"] = focal_metrics["test_accuracy"] - baseline.get("test_accuracy", 0)
        focal_metrics["delta_weighted_f1"] = focal_metrics["test_weighted_f1"] - baseline.get("test_f1_weighted", 0)
        logger.info(f"Delta Macro F1 vs Baseline: {focal_metrics['delta_macro_f1']:+.4f}")

    # Add experiment metadata
    focal_metrics["experiment_metadata"] = {
        **EXPERIMENT_METADATA,
    ...
    with open(os.path.join(RESULTS, "focal_loss_metrics.json"), "w") as f:
        json.dump(focal_metrics, f, indent=2)

    logger.info(f"Test Accuracy : {focal_metrics['test_accuracy']:.4f}")
    logger.info(f"Test Macro F1 : {focal_metrics['test_macro_f1']:.4f}")
    logger.info(f"Neutral F1    : {focal_metrics['neutral_f1']:.4f}")
    logger.info("PHASE 3C-1 COMPLETE.")
